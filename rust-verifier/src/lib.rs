...
impl HeaderV2 {
    ...
    pub fn decode_strict(data: &[u8]) -> Result<Self> {
        ...
        if data[0] != Self::TYPE_HEADER {
            return Err(VerifyError::Protocol(format!("expected header type 0x02, got 0x{:02x}", data[0])));
        }
        ...
        Ok(HeaderV2 { version, prev_hash, merkle_root, registry_root, timestamp, target, nonce })
    }

    pub fn encode(&self) -> [u8; HEADER_V2_LEN] {
        let mut out = [0u8; HEADER_V2_LEN];
        out[0] = Self::TYPE_HEADER;
        ...
        out[off..off+8].copy_from_slice(&self.nonce.to_be_bytes());
        out
    }
}
...
pub fn hash_leq_target(hash: &[u8; HASH_LEN], target: &[u8; HASH_LEN]) -> bool {
    for i in 0..HASH_LEN {
        if hash[i] != target[i] {
...
pub fn target_from_hex(hex: &str) -> Result<[u8; HASH_LEN]> {
    if hex.len() != 64 {
        return Err(VerifyError::Protocol(format!("target hex length {} not 64", hex.len())));
    }
    let mut out = [0u8; HASH_LEN];
...
pub fn build_attestation_preimage(network_id: &str, version: u32, body_hash: &str, curator_id: &str, block_height: u64) -> Value {
    ...
    map.insert("block_height".to_string(), Value::Number(block_height.into()));
    Value::Object(map)
}
