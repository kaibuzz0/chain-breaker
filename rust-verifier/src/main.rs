...
fn as_array(v: &serde_json::Value, context: &str) -> Result<&Vec<serde_json::Value>, VerifyError> {
    v.as_array().ok_or_else(|| VerifyError::Protocol(format!("{}: expected array", context)))
}

fn check_header_v2(dir: &PathBuf) -> Result<(), VerifyError> {
    let val = load_json(dir, "header-v2.json")?;
    for v in as_array(val.get("vectors").ok_or_else(|| VerifyError::Protocol("missing vectors".into()))?, "header-v2")? {
        let bytes = hex::decode(v["canonical_bytes_hex"].as_str().ok_or_else(|| VerifyError::Protocol("missing canonical_bytes_hex".into()))?)?;
        if v["expected_validity"].as_bool().ok_or_else(|| VerifyError::Protocol("missing expected_validity".into()))? {
            let header = HeaderV2::decode_strict(&bytes)?;
            let reencoded = header.encode();
            expect_hex("header encode roundtrip", v["canonical_bytes_hex"].as_str().unwrap(), &reencoded)?;
            let hash = double_sha256(&reencoded);
            let expected = v["expected_hash"].as_str().ok_or_else(|| VerifyError::Protocol("missing expected_hash".into()))?;
            ...
            if HeaderV2::decode_strict(&bytes).is_ok() {
                return Err(VerifyError::Protocol(format!("negative header should fail: {}", v["description"].as_str().unwrap_or(""))));
            }
        }
    }
...
fn check_genesis(dir: &PathBuf) -> Result<(), VerifyError> {
    ...
    expect_hex("genesis bytes", val["expected_header_bytes_hex"].as_str().ok_or_else(|| VerifyError::Protocol("missing expected_header_bytes_hex".into()))?, &bin)?;
    let hash = double_sha256(&bin);
    if hex::encode(hash) != val["expected_header_hash"].as_str().ok_or_else(|| VerifyError::Protocol("missing expected_header_hash".into()))? {
        return Err(VerifyError::Mismatch {
    ...
    if hex::encode(header.registry_root) != val["expected_registry_root"].as_str().ok_or_else(|| VerifyError::Protocol("missing expected_registry_root".into()))? {
        return Err(VerifyError::Mismatch {
...
fn check_sha256d(dir: &PathBuf) -> Result<(), VerifyError> {
    ...
    let input = hex::decode(val["input_hex"].as_str().ok_or_else(|| VerifyError::Protocol("missing input_hex".into()))?)?;
    let got = double_sha256(&input);
    if hex::encode(got) != val["expected_sha256d"].as_str().ok_or_else(|| VerifyError::Protocol("missing expected_sha256d".into()))? {
        return Err(VerifyError::Mismatch {
...
fn check_pow(dir: &PathBuf) -> Result<(), VerifyError> {
    ...
    let expected_be = val["max_target_hex_be"].as_str().ok_or_else(|| VerifyError::Protocol("missing max_target_hex_be".into()))?;
    let expected_le = val["max_target_hex_le"].as_str().ok_or_else(|| VerifyError::Protocol("missing max_target_hex_le".into()))?;
    let max_target = target_from_hex(expected_be)?;
    if hex::encode(max_target) != expected_be {
        return Err(VerifyError::Mismatch { context: "max target BE".into(), expected: expected_be.into(), actual: hex::encode(max_target) });
    }
    let mut le = max_target;
    le.reverse();
    if hex::encode(le) != expected_le {
        return Err(VerifyError::Mismatch { context: "max target LE".into(), expected: expected_le.into(), actual: hex::encode(le) });
    }
    for neg in val.get("negative_vectors").and_then(|v| v.as_array()).unwrap_or(&Vec::new()) {
        let hash = target_from_hex(neg["block_hash_hex"].as_str().ok_or_else(|| VerifyError::Protocol("missing block_hash_hex".into()))?)?;
        let target = target_from_hex(neg["target_hex"].as_str().ok_or_else(|| VerifyError::Protocol("missing target_hex".into()))?)?;
        if hash_leq_target(&hash, &target) {
            return Err(VerifyError::Protocol(format!("PoW negative should fail: {}", neg["description"].as_str().unwrap_or(""))));
        }
    }
    Ok(())
}
...
fn check_merkle(dir: &PathBuf) -> Result<(), VerifyError> {
    for name in ["merkle.json", "merkle-extra.json"] {
        ...
        for v in as_array(val.get("vectors").ok_or_else(|| VerifyError::Protocol(format!("{}: missing vectors", name)))?, name)? {
            ...
            let expected = v["expected_root_hex"].as_str().ok_or_else(|| VerifyError::Protocol("missing expected_root_hex".into()))?;
            if hex::encode(root) != expected {
                return Err(VerifyError::Mismatch {
                    context: format!("merkle {}", v["leaf_count"].as_u64().unwrap_or(0)),
                    expected: expected.into(),
...
fn check_registry_state(dir: &PathBuf) -> Result<(), VerifyError> {
    ...
    expect_hex("registry-state bytes", val["state_hex"].as_str().ok_or_else(|| VerifyError::Protocol("missing state_hex".into()))?, &bin)?;
    let root = sha256_single(&bin);
    if hex::encode(root) != val["expected_registry_root"].as_str().ok_or_else(|| VerifyError::Protocol("missing expected_registry_root".into()))? {
        return Err(VerifyError::Mismatch {
...
fn check_governance_register(dir: &PathBuf) -> Result<(), VerifyError> {
    ...
    let gov_keys: Vec<String> = as_array(&val["vectors"].as_array().unwrap()[0]["governance_keys"], "governance_keys")?
    ...
    for v in as_array(&val["vectors"], "governance-register vectors")? {
        ...
        if v["expected_validity"].as_bool().unwrap_or(false) {
            for sig in sigs {
                let idx = sig["key_index"].as_u64().unwrap_or(0) as usize;
                if idx >= gov_keys.len() {
                    return Err(VerifyError::Protocol("key_index out of range".into()));
                }
                verify_ed25519(&gov_keys[idx], &msg, sig["signature"].as_str().unwrap_or(""))?;
            }
        }
    }
    Ok(())
...
fn check_attestation(dir: &PathBuf) -> Result<(), VerifyError> {
    ...
    for v in as_array(&val["vectors"], "attestation vectors")? {
        let preimage = build_attestation_preimage(
            v["network_id"].as_str().ok_or_else(|| VerifyError::Protocol("missing network_id".into()))?,
            v["protocol_version"].as_u64().unwrap_or(0) as u32,
            v["manifest_hash"].as_str().ok_or_else(|| VerifyError::Protocol("missing manifest_hash".into()))?,
            v["curator_id"].as_str().ok_or_else(|| VerifyError::Protocol("missing curator_id".into()))?,
            v["block_height"].as_u64().unwrap_or(0),
        );
        let msg = hash_object(&preimage);
        let pk = v["curator_public_key_hex"].as_str().ok_or_else(|| VerifyError::Protocol("missing curator_public_key_hex".into()))?;
        let sig = v["expected_signature_hex"].as_str().ok_or_else(|| VerifyError::Protocol("missing expected_signature_hex".into()))?;
        let should_pass = v["expected_validity"].as_bool().ok_or_else(|| VerifyError::Protocol("missing expected_validity".into()))?;
...
fn check_ed25519(dir: &PathBuf) -> Result<(), VerifyError> {
    ...
    let pk = val["public_key_hex"].as_str().ok_or_else(|| VerifyError::Protocol("missing public_key_hex".into()))?;
    let msg = hex::decode(val["message_hex"].as_str().ok_or_else(|| VerifyError::Protocol("missing message_hex".into()))?)?;
    let sig = val["signature_hex"].as_str().ok_or_else(|| VerifyError::Protocol("missing signature_hex".into()))?;
    let expected = val["expected_validity"].as_bool().ok_or_else(|| VerifyError::Protocol("missing expected_validity".into()))?;
    let ok = verify_ed25519(pk, &msg, sig).is_ok();
    if ok != expected {
        return Err(VerifyError::Protocol("ed25519 expected validity mismatch".into()));
    }
    Ok(())
}
...
fn check_block(dir: &PathBuf) -> Result<(), VerifyError> {
    ...
    let expected = val["expected_header_hash"].as_str().ok_or_else(|| VerifyError::Protocol("missing expected_header_hash".into()))?;
    if hex::encode(hash) != expected {
        return Err(VerifyError::Mismatch {
...
fn parse_header_from_dict(dict: &serde_json::Value) -> Result<HeaderV2, VerifyError> {
    Ok(HeaderV2 {
        ...
        prev_hash: hex_to_hash(dict["prev_hash"].as_str().ok_or_else(|| VerifyError::Protocol("missing prev_hash".into()))?)?,
        merkle_root: hex_to_hash(dict["merkle_root"].as_str().ok_or_else(|| VerifyError::Protocol("missing merkle_root".into()))?)?,
        registry_root: hex_to_hash(dict["registry_root"].as_str().ok_or_else(|| VerifyError::Protocol("missing registry_root".into()))?)?,
        timestamp: dict["timestamp"].as_u64().unwrap_or(0),
        target: hex_to_hash(dict["target"].as_str().ok_or_else(|| VerifyError::Protocol("missing target".into()))?)?,
        nonce: dict["nonce"].as_u64().unwrap_or(0),
    })
}

fn hex_to_hash(s: &str) -> Result<[u8; HASH_LEN], VerifyError> {
    let mut out = [0u8; HASH_LEN];
    hex::decode_to_slice(s, &mut out)?;
    Ok(out)
}

fn remove_witness_keys(mut value: serde_json::Value) -> serde_json::Value {
