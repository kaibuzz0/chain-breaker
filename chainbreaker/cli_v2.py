...
from .network_identity import NetworkIdentityError
...
def _load_ledger(path: Path) -> Ledger:
    ...
    if genesis_header.get("prev_hash") != "0" * 64:

        raise CLIError("ledger genesis must have prev_hash 0")

    try:

        return Ledger.from_dict(raw)

    except (LedgerError, NetworkIdentityError, ValueError, KeyError, TypeError) as exc:
...
def v2_genesis() -> None:
    ...
    output = {
        ...
        "note": "This is the frozen Protocol V2 alpha/dev/test identity. "

                "Production deployments must create a new network identity.",
...
@click.option("--dev", is_flag=True, help="Use the frozen alpha/dev/test identity with placeholder governance keys")

@click.option("--governance-key", "-g", multiple=True, type=click.Path(exists=True), help="Path to a governance private key file (hex); required for production; may be used for explicit test networks")

@click.option("--governance-threshold", "-t", type=int, default=None, help="Required governance signatures (default: majority of provided keys)")

@click.option("--network-id", "-n", type=str, default=None, help="Explicit network_id for a production/test network (default: derived for test, required for production)")
...
def v2_chain_init(
    ...
    governance_key: tuple[str, ...],

    governance_threshold: int | None,

    network_id: str | None,

) -> None:

    """Create a local v2 ledger initialized from a network identity.



    Without options, this creates a ledger using the frozen alpha/dev/test

    identity (placeholder governance keys).



    Use ``--dev`` to make that choice explicit.  For production or isolated

    test networks, provide governance private key files with ``--governance-key``.
    ...
    if dev and governance_key:

        raise CLIError("--dev cannot be combined with --governance-key")



    if governance_key:

        # Production / explicit key-ceremony path.

        private_hexes = []

        for key_path in governance_key:

            raw = _safe_read(_resolve_path(key_path)).decode("utf-8").strip()

            # Accept a 64-char hex private key; reject obvious placeholder keys.

            if len(raw) < 64:

                raise CLIError(f"governance private key file is too short: {key_path}")

            sk_hex = raw[:64]

            if sk_hex in GENESIS_GOVERNANCE_KEYS:

                raise CLIError(

                    f"governance private key file contains an alpha placeholder key: {key_path}"

                )

            private_hexes.append(sk_hex)



        public_keys = []

        for sk_hex in private_hexes:

            try:

                sk = decode_private_key(sk_hex)

            except ValueError as exc:

                raise CLIError(f"invalid governance private key: {exc}") from exc

            pk = encode_public_key(sk.public_key())

            public_keys.append(pk)



        if tuple(sorted(public_keys)) == tuple(sorted(GENESIS_GOVERNANCE_KEYS)):

            raise CLIError(

                "production/test identity cannot derive from the alpha placeholder governance key set"
        ...
        if not public_keys:
            ...
            raise CLIError("at least one --governance-key is required for production/test initialization")

        if governance_threshold is None:

            governance_threshold = max(1, len(public_keys) // 2)

        if not (1 <= governance_threshold <= len(public_keys)):

            raise CLIError(

                f"--governance-threshold must satisfy 1 <= threshold <= {len(public_keys)}"
        ...
        if network_id is None:
            ...
            # Derive a deterministic test network_id.

            from .network_identity import derive_test_network_identity

            identity = derive_test_network_identity(public_keys, governance_threshold)

            network_id = identity.network_id

        else:

            from .network_identity import derive_network_identity
        ...
        if governance_threshold is not None:
            ...
            raise CLIError("--governance-threshold requires --governance-key")

        if network_id is not None:

            raise CLIError("--network-id requires --governance-key")
    ...
    click.echo(json.dumps({
        ...
        "kind": ledger.network_identity.kind,
...
def v2_chain_verify(ledger: str) -> None:
    ...
    output = {
        ...
        "network_id": led.network_id,

        "kind": led.network_identity.kind,
