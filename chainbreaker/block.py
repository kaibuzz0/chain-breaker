...
def mine_header_v2(header: BlockHeaderV2,
    ...
    The fixed alpha genesis header is a protocol constant and must never be

    re-mined.  If a caller passes the exact alpha genesis header, this

    function rejects the attempt rather than silently returning.

    """

    # ``start_nonce`` is accepted for API compatibility but is ignored: the

    # search begins from the current header nonce so that wrapping behavior

    # remains predictable for callers.

    if header.prev_hash == "0" * 64 and header.version == PROTOCOL_VERSION:

        from chainbreaker.codec import BinaryCodec

        candidate = BinaryCodec.encode_header_v2(header.to_dict())

        if candidate == GENESIS_HEADER_BYTES:
...
def _compute_genesis_constants() -> BlockV2:
    ...
    """Removed. Use ``create_genesis_block`` for the frozen alpha identity or

    ``derive_network_identity`` in ``chainbreaker.network_identity`` to
    ...
    raise RuntimeError(
        ...
        "_compute_genesis_constants is removed: "

        "use create_genesis_block() for the frozen alpha genesis or "

        "derive_network_identity() to create a new V2 network identity"
