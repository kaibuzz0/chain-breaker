...
class ReorgEngine:
    ...
    def __init__(
        ...
        self.network_identity = network_identity or self._infer_network_identity()



    def _infer_network_identity(self) -> NetworkIdentity:

        """Return an identity consistent with the current chain's genesis.



        If the genesis matches the frozen alpha identity, use that.  Otherwise

        derive a deterministic test identity from the configured governance keys.

        """

        if not self.current_chain:

            return alpha_network_identity()

        genesis = self.current_chain[0]

        if identity_matches_genesis(alpha_network_identity(), genesis):

            return alpha_network_identity()

        return derive_test_network_identity(self.governance_keys, self.governance_threshold)
